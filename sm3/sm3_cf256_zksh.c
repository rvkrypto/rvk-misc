...
//	=== The Chinese Standard SM3 Hash -- Compression Function (non-vector)
...
#define STEP_SM3_KEY(w0, w3, w7, wa, wd) {	\
	t = w0 ^ w7 ^ __riscv_ror_32(wd, 17);	\
	t = __riscv_sm3p1(t);					\
	w0 = wa ^ __riscv_ror_32(w3, 25) ^ t;	}

//	rounds 0..15

#define STEP_SM3_RF0(a, b, c, d, e, f, g, h, w0, w4) {	\
	h = h + w0;											\
	t = __riscv_ror_32(a, 20);							\
	u = t + e + tj;										\
	u = __riscv_ror_32(u, 25);							\
	d = d + (t ^ u) + (a ^ b ^ c);						\
	b = __riscv_ror_32(b, 23);							\
	h = h + u + (e ^ f ^ g);							\
	h = __riscv_sm3p0(h);								\
	f = __riscv_ror_32(f, 13);							\
	d = d + (w0 ^ w4);									\
	tj = __riscv_ror_32(tj, 31);	}
