...
{
	z->w[0] = __riscv_brev8_32(z->w[0]);
	z->w[1] = __riscv_brev8_32(z->w[1]);
	z->w[2] = __riscv_brev8_32(z->w[2]);
	z->w[3] = __riscv_brev8_32(z->w[3]);
}
...
void ghash_mul_rv32(gf128_t * z, const gf128_t * x, const gf128_t * h)
{
	...
	//	4 x GREV
	x0 = __riscv_brev8_32(x0);				//	reverse input x only
	x1 = __riscv_brev8_32(x1);
	x2 = __riscv_brev8_32(x2);
	x3 = __riscv_brev8_32(x3);
	...
	z4 = __riscv_clmulh_32(x3, y);
	z3 = __riscv_clmul_32(x3, y);
	t1 = __riscv_clmulh_32(x2, y);
	z2 = __riscv_clmul_32(x2, y);
	z3 = z3 ^ t1;
	t1 = __riscv_clmulh_32(x1, y);
	z1 = __riscv_clmul_32(x1, y);
	z2 = z2 ^ t1;
	t1 = __riscv_clmulh_32(x0, y);
	z0 = __riscv_clmul_32(x0, y);
	z1 = z1 ^ t1;

#ifdef NO_SHIFTRED
	//	Mul reduction: 1 x CLMULH, 1 x CLMUL, 2 x XOR
	t1 = __riscv_clmulh_32(z4, 0x87);
	t0 = __riscv_clmul_32(z4, 0x87);
	z1 = z1 ^ t1;
	z0 = z0 ^ t0;
#else
	...
	for (i = 2; i >= 0; i--) {				//	towards less significant
		...
		t1 = __riscv_clmulh_32(x3, y);
		t0 = __riscv_clmul_32(x3, y);
		z4 = z3 ^ t1;
		t1 = __riscv_clmulh_32(x2, y);
		t2 = __riscv_clmul_32(x2, y);
		z3 = z2 ^ t0 ^ t1;
		t1 = __riscv_clmulh_32(x1, y);
		t0 = __riscv_clmul_32(x1, y);
		z2 = z1 ^ t1 ^ t2;
		t1 = __riscv_clmulh_32(x0, y);
		t2 = __riscv_clmul_32(x0, y);
		z1 = z0 ^ t0 ^ t1;
...
void ghash_mul_rv32_kar(gf128_t * z, const gf128_t * x, const gf128_t * h)
{
	...
	z7 = __riscv_clmulh_32(x3, y3);				//	high pair
	z6 = __riscv_clmul_32(x3, y3);
	z5 = __riscv_clmulh_32(x2, y2);
	z4 = __riscv_clmul_32(x2, y2);
	t0 = x2 ^ x3;
	t2 = y2 ^ y3;
	t1 = __riscv_clmulh_32(t0, t2);
	t0 = __riscv_clmul_32(t0, t2);
	t1 = t1 ^ z5 ^ z7;
	t0 = t0 ^ z4 ^ z6;
	z6 = z6 ^ t1;
	z5 = z5 ^ t0;

	z3 = __riscv_clmulh_32(x1, y1);				//	low pair
	z2 = __riscv_clmul_32(x1, y1);
	z1 = __riscv_clmulh_32(x0, y0);
	z0 = __riscv_clmul_32(x0, y0);
	t0 = x0 ^ x1;
	t2 = y0 ^ y1;
	t1 = __riscv_clmulh_32(t0, t2);
	t0 = __riscv_clmul_32(t0, t2);
	...
	x3 = __riscv_clmulh_32(t1, t3);				//	middle
	x2 = __riscv_clmul_32(t1, t3);
	x1 = __riscv_clmulh_32(t0, t2);
	x0 = __riscv_clmul_32(t0, t2);
	...
	t1 = __riscv_clmulh_32(z7, 0x87);
	t0 = __riscv_clmul_32(z7, 0x87);
	z4 = z4 ^ t1;
	z3 = z3 ^ t0;
	t1 = __riscv_clmulh_32(z6, 0x87);
	t0 = __riscv_clmul_32(z6, 0x87);
	z3 = z3 ^ t1;
	z2 = z2 ^ t0;
	t1 = __riscv_clmulh_32(z5, 0x87);
	t0 = __riscv_clmul_32(z5, 0x87);
	z2 = z2 ^ t1;
	z1 = z1 ^ t0;
	t1 = __riscv_clmulh_32(z4, 0x87);
	t0 = __riscv_clmul_32(z4, 0x87);
