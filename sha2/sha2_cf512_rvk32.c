//	sha2_cf512_rvk32.c
//	2020-03-08	Markku-Juhani O. Saarinen <mjos@iki.fi> See LICENSE.
...
static inline int32_t __riscv_sltu(int32_t rs1, int32_t rs2)
{
	int32_t rd;
...
#define STEP_SHA512_K(i) {												\
	...
	ul = __riscv_sha512sig0l(mp[(i + 2) & 0x1F], mp[(i + 3) & 0x1F]);	\
	uh = __riscv_sha512sig0h(mp[(i + 3) & 0x1F], mp[(i + 2) & 0x1F]);	\
	STEP_ADD64(tl, th, tl, th, ul, uh);									\
	ul = __riscv_sha512sig1l(mp[(i + 28) & 0x1F], mp[(i + 29) & 0x1F]); \
	uh = __riscv_sha512sig1h(mp[(i + 29) & 0x1F], mp[(i + 28) & 0x1F]); \
	STEP_ADD64(tl, th, tl, th, ul, uh);									\
	mp[i] = tl;															\
	mp[i + 1] = th; }
...
#define STEP_SHA512_R(x0, x1, x2, x3, x4, x5, x6, x7,	\
	...
	th = (xd ^ (x9 & (xb ^ xd)));			\
	STEP_ADD64(xe, xf, xe, xf, tl, th);		\
	tl = __riscv_sha512sum1r(x8, x9);		\
	th = __riscv_sha512sum1r(x9, x8);		\
	STEP_ADD64(xe, xf, xe, xf, tl, th);		\
	STEP_ADD64(x6, x7, x6, x7, xe, xf);		\
	tl = __riscv_sha512sum0r(x0, x1);		\
	th = __riscv_sha512sum0r(x1, x0);		\
	STEP_ADD64(xe, xf, xe, xf, tl, th);		\
	tl = (((x0 | x4) & x2) | (x4 & x0));	\
	th = (((x1 | x5) & x3) | (x5 & x1));	\
	STEP_ADD64(xe, xf, xe, xf, tl, th); }
